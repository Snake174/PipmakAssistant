 ...
 */
    ...
    QString filename = QString::fromLatin1( buffer2, szName ); //::fromAscii(buffer2, szName);
    if (filename != path) {
...
*/
  UnZip::ErrorCode UnzipPrivate::parseCentralDirectoryRecord()
  {
    ...
    if (device->read(buffer2, szName) != szName) {
      ...
      filename = QString::fromLatin1( buffer2, szName ); //::fromAscii(buffer2, szName);
    }
    ...
    if (szComment != 0) {
      ...
      h->comment = QString::fromLatin1( buffer2, szComment ); //::fromAscii(buffer2, szComment);
    }
 ...
 */
  void UnzipPrivate::initKeys(const QString& pwd, quint32* keys) const
  {
    ...
    QByteArray pwdBytes = pwd.toLatin1(); //.toAscii();
